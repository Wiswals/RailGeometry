/***************************************************************************************************
Procedure:          Calculate Live Track Geometry
...
Affected table(s):  [TrackGeometry].[dbo.TrackListing]
                    [TrackGeometry].[dbo.GeometryData]
                    [TrackGeometry].[dbo.PrismData]
                    [TrackGeometry].[dbo.TrackGeometry]
Affected function:  [TrackGeometry].[dbo.ParseString]
                    [TrackGeometry].[dbo.BearingandDistance]
Used By:            N/A 
...
2019-09-24			Lewis Walsh			Created data storage routine for GeometryData and PrismData
...
2024-12-13			Lewis Walsh			Added debug more 3 for independent parameter calculations without altering 
                                        database tables
2025-04-11          Lewis Walsh         v0.6 updates: Added averaging range thresholds and minimum point count filter,
                                        revised print/log outputs with new config parameters, enforced both-left-and-right
                                        prism spacing for valid calculations, and added debug output blocks to aid testing
***************************************************************************************************/
...
--===============================================================================================================================
--Debug Settings
	DECLARE @Debug as int = 3								-- If set to	0 = live automation from exe (Restrain results, allow dynamic variable setting), 
															--				1 = Psuedo live (Print Results, keep data), 
															--				2 = Reset mode (Print results, clear and reset tables)
															--				3 = Independent mode (Print results, dont store to any tables)
...
--Calculation Settings
	...
	DECLARE @DataExtractionWindow as int = 800				-- Number of hours to look back when extracting data for the calculations
	DECLARE @AverageData as bit = 1;						-- Identifier to enable or disable averaging of extracted data (1 = enable, 0 = disable)
	DECLARE @MinPointCount INT = 2; 						-- Minimum number of points required for a calculation to run.
	DECLARE @MaxRange_E FLOAT = 0.015; 						-- Range tolerance check for Easting extraction (only applies to averaging approach)
	DECLARE @MaxRange_N FLOAT = 0.015; 						-- Range tolerance in Northing extraction (only applies to averaging approach)
	DECLARE @MaxRange_H FLOAT = 0.010; 						-- Range tolerance in Height extraction (only applies to averaging approach)
	DECLARE @PrismSpacingLimit as decimal(38,10) = 8		-- A value (in meters) used to check the spacing between track prisms any prisms separated by more than this value will not get geometry calculations.
...
--=============================================================================================================================== 
	...
	PRINT ' - Right Rail Indicator: ' + CAST(@RightRailIndicator as varchar) + ' | String identifier for right rail points. Must match with right rail names from GeoMoS.'
	PRINT ' - Minimum point count: ' + CAST(@MinPointCount AS varchar) + ' | Skip point groups with fewer observations than this value.'
	PRINT ' - Range thresholds (mm): E ≤ ' + CAST(@MaxRange_E * 1000 AS varchar) + 
			', N ≤ ' + CAST(@MaxRange_N * 1000 AS varchar) + 
			', H ≤ ' + CAST(@MaxRange_H * 1000 AS varchar) + ' | Exclude point groups exceeding these tolerances.' + CHAR(13)
	...
	BEGIN TRY
		SET @Database_Script = 'USE [Automated Track Geometry]
								SELECT ROW_NUMBER() OVER(ORDER BY [Track_Name] ASC) AS Row_ID, 
									   ...
									   [Start_Chainage], 
									   [End_Chainage],
									   (SELECT MAX([Calculation_ID]) FROM [GeometryData]) AS [Calculation_ID]
		...
		/*-- Troubleshooting Code Block
			-- Prints the calculation listing table
	...
	BEGIN
		...
		IF	@CurrentDateTime > DateADD(MINUTE, @CalculationFrequency, @LastRailCalcTime)  --AND @ExpectedDatabase = DB_NAME() 
			BEGIN
					...
					IF @AverageData = 1
						BEGIN
							BEGIN TRY
								...
								HAVING	(MAX(Results.Easting) - MIN(Results.Easting)) < @MaxRange_E
										AND (MAX(Results.Northing) - MIN(Results.Northing)) < @MaxRange_N
										AND (MAX(Results.Height) - MIN(Results.Height)) < @MaxRange_H
										AND COUNT(Points.Name) >= @MinPointCount
							END TRY
							BEGIN CATCH
					...
					BEGIN
						...
						WHILE @ChainageIndex <= @CalcEndChainage
							BEGIN
								...
								--Reset collection variables to avoid overlapping Left and right rail data
								...
								SET @PrismComment = 'LR Start: ' + ISNULL(@Prev_PointName, '[Invalid]') + ' | LR End: ' + ISNULL(@Next_PointName, '[Invalid]')

								/*-- Troubleshooting Code Block
									-- Prints the left rail prism set data
									Print ''
									Print @ChainageIndex
									Print '	Left Rail: '
									Print '		Srt: ' + @Prev_PointName
									Print '		End: ' + @Next_PointName
									Print '		Chn: ' + FORMAT(@Diff_Chainage_Left, 'N1')
									Print @PrismComment
									Print ''
								*/
								...
								SET @PrismComment = @PrismComment + ' | RR Start: ' + ISNULL(@Prev_PointName, '[Invalid]') + ' | RR End: ' + ISNULL(@Next_PointName, '[Invalid]')


								/*-- Troubleshooting Code Block
									-- Prints the right rail prism set data
									Print '	Right Rail'
									Print '		Srt: ' + @Prev_PointName
									Print '		End: ' + @Next_PointName
									Print '		Chn: ' + FORMAT(@Diff_Chainage_Rght, 'N1')
									Print @PrismComment
									Print ''
									Print '-----------------------------------------------'
								*/
								...
								IF @Diff_Chainage_Left <= @PrismSpacingLimit AND @Diff_Chainage_Rght <= @PrismSpacingLimit
									BEGIN	
				...
				--===============================================================================================================================================
				IF @Calculation_Check = 1 AND (@Debug = 0 OR @Debug = 1 OR @Debug = 2)
					BEGIN
						...
						--Store the current round of prism data into the PrismData table
						BEGIN TRY
							SET @Database_Script =	'USE [Automated Track Geometry]
													INSERT INTO PrismData SELECT [Calculation_ID], [Point_Name], [Point_Epoch], [Point_Group], [Point_ExpTime_DD],  
													[Point_Easting], [Point_Northing], [Point_Height], [Point_EOffset], [Point_NOffset], [Point_HOffset], [Track_Chainage], [Track_Section],
													[Track_RailSide], [Track_Code], [Track_Easting], [Track_Northing], [Track_Height], [Epoch_Index], [Point_Remark] FROM #PrismData'
							EXEC (@Database_Script)
							PRINT '   ' + CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | Writing to [PrismData] table completed.'
						END TRY
						BEGIN CATCH
							PRINT '   ' + CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | Writing to [PrismData] table failed: ' + ERROR_MESSAGE();
						END CATCH;
						
						--Store the current round of geometry data into the GeometryData table
						BEGIN TRY
							SET @Database_Script =	'USE [Automated Track Geometry]
													INSERT INTO GeometryData SELECT	[Calculation_ID], [Track_Code], [Track_Section], [Track_Code] + ''-'' + FORMAT(Track_CL_Chainage,'''+ @ChainageNameFormat +''') as [Geometry_Instrument],
							...
							EXEC (@Database_Script)
							PRINT '   ' + CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | Writing to [GeometryData] table completed.'
						END TRY
						BEGIN CATCH
							PRINT '   ' + CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | Writing to [GeometryData] table failed: ' + ERROR_MESSAGE();
						END CATCH;
						
						--===============================================================================================================================================
						-- 3.4.2 Drop the #PrismData and #TrackGeometry tabled in preperation for the next round of calculations
						--===============================================================================================================================================
						
						-- Depending on selected operation mode, reset all calculation tables
						IF @Debug = 1
							BEGIN
								--Debug mode enabled - show user calculated results. 
								SELECT * FROM #PrismData
								SELECT * FROM #TrackGeometry
								PRINT '   ' + CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | Script running in debug mode (1) - Psuedo live. [GeometryData] and [PrismData] tables have been written to.';
							END
						...
						IF @Debug = 2
							BEGIN
								...
								SELECT * FROM #PrismData
								SELECT * FROM #TrackGeometry
								--Enable the code below to clear the Geometry history and the PrismData tables
								SET @Database_Script =	'USE [Automated Track Geometry] DELETE FROM GeometryData DELETE FROM PrismData'
								EXEC (@Database_Script)
								PRINT '   ' + CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | Script running in debug mode (2) - Reset mode. [GeometryData] and [PrismData] tables have been cleared and reset.';
							END
					END
                ELSE
					BEGIN
						IF @Calculation_Check = 1 AND @Debug = 3
							BEGIN
								-- Runs if Debug mode 3 is selected - show user calculated results. 
									SELECT [Calculation_ID], [Point_Name], [Point_Epoch], [Point_Group], [Point_ExpTime_DD],  
															[Point_Easting], [Point_Northing], [Point_Height], [Point_EOffset], [Point_NOffset], [Point_HOffset], [Track_Chainage], [Track_Section],
															[Track_RailSide], [Track_Code], [Track_Easting], [Track_Northing], [Track_Height], [Epoch_Index], [Point_Remark] FROM #PrismData
									
									SELECT	[Calculation_ID], [Track_Code], [Track_Section], [Track_Code] + '-' + FORMAT(Track_CL_Chainage, @ChainageNameFormat) as [Geometry_Instrument],
															[DataWindow_End] as [Calculation_Epoch], [Track_CL_Chainage] AS [Calculation_Chainage], [DataWindow_Start], [DataWindow_End], [Rail_Cant], [Rail_Gauge],
															[Twist_Short], [Twist_Long], [LR_ID], [LR_Easting], [LR_Northing], [LR_Height], [LR_Radius], [LR_Top_Short], [LR_Top_Long], [LR_Line_Short], [LR_Line_Long],
															[RR_ID], [RR_Easting], [RR_Northing], [RR_Height], [RR_Radius], [RR_Top_Short], [RR_Top_Long], [RR_Line_Short], [RR_Line_Long], [CL_ID], [CL_Easting],
															[CL_Northing], [CL_Height], [CL_Radius], [CL_Top_Short], [CL_Top_Long], [CL_Line_Short], [CL_Line_Long], [Calculation_Comment], [Prism_Comment] AS [Prism_Inputs],
									...
									PRINT '   ' + CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | Script running in debug mode (3) - Independent mode. [GeometryData] and [PrismData] tables have not been altered.'
							END
						ELSE 
							GOTO ExitScript
					END
...
PRINT '   ' + CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | Track Geometry script completed.';
