...
DECLARE @CalcInterval as Int
DECLARE @SearchWindow as Int
...
SELECT @StartTimeHistory = '20170908 00:00', @EndTimeHistory = '20170909 00:....', @CalcInterval=30, @SearchWindow=96
...
BEGIN
	...
	BEGIN
		...
		SET @EndTimeCalc=@NextRailCalcTime
		SET @StartTimeCalc=DateADD(hh,-@SearchWindow,@EndTimeCalc)
	...
	--Create table of all readings between the start and end time
	INSERT INTO #MyTableALL
	SELECT      Points.Name, Results.Epoch AS Date, 
				(Results.Easting) AS Easting, (Results.Northing)  AS Northing, (Results.Height)  AS Height, 
				Coordinates.Easting  AS EastNULL, Coordinates.Northing  AS NorthNULL, Coordinates.Height  AS HeightNULL, 
				(Results.Easting) - Coordinates.Easting AS dE, (Results.Northing) - Coordinates.Northing  AS dN, (Results.Height) - Coordinates.Height  AS dH, 
				(Results.LongitudinalDisplacement)  AS dLong, (Results.TransverseDisplacement)  AS dTran, (Results.HeightDisplacement)  AS dHgt, 
				Profiles.Azimuth as Profile, 
				([dbo].[ParseString](Points.Description,'CH:')) AS Chainage,
				substring(Points.Name,4,1) AS Rail,
				([dbo].[ParseString](Points.Description,'X:')) AS XOffset,
				([dbo].[ParseString](Points.Description,'Y:')) AS YOffset, 
	...
	WHERE       (Results.Type = 0) AND (PointGroups.Name LIKE N'Track%') AND 
				(Results.Epoch >= @StartTimeCalc) AND (Results.Epoch < @EndTimeCalc) AND (Coordinates.Type = 1)
					AND (Points.Name LIKE @myTrack + '%' )
	ORDER BY	Chainage, Points.Name

	--Create table of latest available readings between start and end time
	SELECT * INTO #MyTable FROM #MyTableALL WHERE 1=2
	INSERT INTO #MyTable SELECT ID,Epoch, Easting, Northing, Height, EastingNULL, NorthingNULL, HeightNULL, dE, dN, dH, dLong, dTran, dHgt, Profile, Chainage, Rail, XOffset, YOffset, EastingShifted, NorthingShifted, 
				HeightShifted, EastingShiftedNULL, NorthingShiftedNULL, HeightShiftedNULL, RailType FROM (SELECT ID,Epoch, Easting, Northing, Height, EastingNULL, NorthingNULL, HeightNULL, dE, dN, dH, dLong, dTran, dHgt, Profile, Chainage, Rail, XOffset, YOffset, EastingShifted, NorthingShifted, 
				HeightShifted, EastingShiftedNULL, NorthingShiftedNULL, HeightShiftedNULL, RailType, row_number() over(partition by ID order by Epoch desc) as rn
    FROM		#MyTableALL) as T 
	WHERE		 rn = 1
	ORDER BY	Rail, Chainage
	...
	--SELECT * FROM #MyTable
	Print 'Processing Track ' + @myTrack + ' @ ' + CONVERT(varchar, @EndTimeCalc, 100) 
			...
			BEGIN
				...
				IF @I>=@MinChainage +5 AND @I<= (Select MAX(ID) from #myTrackPoints)-5
					BEGIN
						...
						SELECT @BearingLongChord=Bearing  from dbo.BandD(@I-5,@Val1,@I+5,@Val3)
						SELECT @BearingMidChord=Bearing  from dbo.BandD(@I-5,@Val1,@I,@Val2)
						SET @dAngle = @BearingLongChord-@BearingMidChord
		...
		SET @NextRailCalcTime=DateADD(minute,@CalcInterval,@NextRailCalcTime)
	END
