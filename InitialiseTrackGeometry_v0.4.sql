...
Affected table(s):  [dbo.TrackListing], [dbo.GeometryData], [dbo.GeometryHistory], [dbo.PrismData], 
                    [dbo.PrismHistory]
...
---------------------------------------------- Create GeometryData Table ----------------------------------------------
-- Used for storing all calculated track geometry parameters. Can be linked to the PrismData table via the [Calculation_ID] column.
IF OBJECT_ID(N'dbo.GeometryData', N'U') IS NULL
	BEGIN
		CREATE TABLE GeometryData (
            [Calculation_ID] INT,                             -- Calculation reference ID
            [Track_Code] VARCHAR(100) NULL,                   -- Track code identifier
			[Track_Section] VARCHAR(100) NULL,                -- Track section identifier
		...
		PRINT CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | GeometryData table created.';
	END
ELSE
	PRINT CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | GeometryData table already exists - no changes made to database.';

---------------------------------------------- Create PrismData Table ----------------------------------------------
-- Used for storing all prism data used for interpolation and computation of track geometry parameters. Can be linked to GeometryData via the [Calculation_ID] column.
IF OBJECT_ID(N'dbo.PrismData', N'U') IS NULL
	BEGIN
		CREATE TABLE PrismData (
			[Calculation_ID] INT,                             -- Calculation reference ID
		...
		PRINT CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | PrismData table created.';
	END
ELSE
	PRINT CONVERT(VARCHAR, GETDATE(), 103) + ' ' + CONVERT(VARCHAR, GETDATE(), 14) + ' | PrismData table already exists - no changes made to database.';
...
-- Note, this table is temporary in nature and is cleared each time the historic track geometry script is run. 
